...
public partial class admin_block_UploadImageMobile : System.Web.UI.UserControl
{
    ...
    public void ShowProduct()
    {
        ...
        if (Session["SSListMobile"] == null)
        {
            ViewProduct.SetPageSize(10);
            ViewProduct.SetIdType(int.Parse(Application["idtypemobile"].ToString()));
            if (text.Equals("-1"))
            {
            ...
            Session["SSListMobile"] = ViewProduct;
        }
        else
        {
            ViewProduct = (CDynamicViewProduct)Session["SSListMobile"];
            if (text.Equals("-1"))
            {
                //ViewProduct.BuildWhere();
            }
        ...
        for (int i = 1; i <= num; i++)
        {
            ...
            strlist += "<td><a href='?menu=updateDes&back=imgmobile&id=" + ds.Tables[0].Rows[i - 1]["Id"].ToString() + "'>" + ds.Tables[0].Rows[i - 1]["Name"].ToString() + "</a></td>";
            ...
            if (url.Length > 0)
            {
                url = "<img class='imgpro' src='../DTDD/image/img_pro/" + url + "' onclick=\"OnChoicePro(" + ds.Tables[0].Rows[i - 1]["Id"].ToString() + ",event,'" + nameImage + "','" + ds.Tables[0].Rows[i - 1]["Name"].ToString() + "');\" />";
            }
            else
            {
                url = "<img class='imgpro' src='../DTDD/image/common/notimgpro.png' onclick=\"OnChoicePro(" + ds.Tables[0].Rows[i - 1]["Id"].ToString() + ",event,'" + nameImage + "','" + ds.Tables[0].Rows[i - 1]["Name"].ToString() + "');\" />";
            }
    ...
    public void BuildPage(int currentpage, int pages)
    {
        for (int i = 1; i <= pages; i++)
        {
            ...
            {
                strPage += "<a href='?menu=imgmobile&page=" + i + "'>" + i + "</a> ";
            }
        }
    }
        ...
        try
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        string _path = Server.MapPath("../DTDD/image/img_pro/");
                        string namepro = "_";
