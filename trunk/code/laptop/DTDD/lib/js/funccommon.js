...
function PageAll(Obj,page,id)
{
    ...
    if(id==2)
    {
        url="?menu=com&page="+ page+ "&size=" + Obj.value;
    }else if(id==3)
    {
        url="?menu=other&page="+ page+ "&size=" + Obj.value;
    }else if(id==4)
    {
         url="?menu=wh&page="+ page+ "&size=" + Obj.value;
    }
    else
    {
        url="?menu=product&page="+ page+ "&size=" + Obj.value;
    }
    location.href=url;
}
function PageBrand(Obj,brand,page,id)
{
    ...
    if(id==1)
    {
        url="?menu=pro&brand=" + brand + "&page="+ page+ "&size=" + Obj.value;
    }else
    {
        url="?menu=igc&id=" + brand + "&page="+ page+ "&size=" + Obj.value;
    }
    location.href=url;
}
...
function OnQuickSearch()
{
    ...
    {
        location.href="?menu="+ select +"&text="+ textSearch;
    }else
    {
        if(select=="dasp")
        {
        ...
        {
            location.href="?menu=com";
        }else if(select=="qsa")
        {
            location.href="?menu=article";
        }
        else if(select=="qsh")
        {
            location.href="?menu=help";
        }
    }
}
function GoGage(index,id)
{
   ...
   if(page!='NaN')
   {
        location.href="?menu="+ menu +"&page="+ page+ "&size=" + ObjSize;
   }
}
function GoGageB(brand,index,id)
{
   ...
   if(page!='NaN')
   {
        if(id==1)
        {
            location.href="?menu=pro&brand="+ brand +"&page="+ page+ "&size=" + ObjSize;
        }else
        {
            location.href="?menu=igc&id="+ brand +"&page="+ page+ "&size=" + ObjSize;
        }
   }
}
function PageS(Obj,page,id)
{
    ...
    if(id==1)
    {
        url="?menu=qsp&text=" + strTextSearch + "&page="+ page+ "&size=" + Obj.value;
    }else if(id==2)
    {
        url="?menu=qsc&text=" + strTextSearch + "&page="+ page+ "&size=" + Obj.value;
    }else if(id==3)
    {
        url="?menu=dasp&advance=1&page="+ page+ "&size=" + Obj.value;
    }
    location.href=url;
}
function GoGageS(index,id)
{
   ...
   if(page!='NaN')
   {
        if(id==1)
        {
            location.href="?menu=qsp&text=" + strTextSearch + "&page="+ page+ "&size=" + ObjSize;
        }else if(id==2)
        {
            location.href="?menu=qsc&text=" + strTextSearch + "&page="+ page+ "&size=" + ObjSize;
        }else if(id==3)
        {
            location.href="?menu=dasp&advance=1&page="+ page+ "&size=" + ObjSize;
        }
   }
}
...
function comp()
{
    ...
    if(arrchoice[1]>0)
    {
       location.href="?menu=compare&id=" + arrchoice[0] + "," + arrchoice[1];
    }else if(arrchoice[0]>0)
    {
        location.href="?menu=compare&id=" + arrchoice[0];
    }
}
function back()
{
...
function gocomp(id)
{
    location.href="?menu=compare&id=" + id;
}
function GotoCart()
{
    location.href="?menu=shoppingcart";
}
function WaitLogin()
{
                    ...
                    if(xhr.status  == 200)
                    {
                        ...
                        if(index==1)
                        {
                            if(str.length>0)
                            {
                                ...
                                var strTable="<div class='td_bd1'><table border='0' cellpadding='0' cellspacing='0' width='100%'>";	
...
{
    var url="?menu=addpro&id=" + id  + "&type=" + type;
    location.href=url;
}
function RequestCommon(index)
{
    var xhr=null; 
    if(window.ActiveXObject)
    { 
...
//For Mobile: Date: 1/12/2008
function OnCPage(menu,page,size,idbrand)
{
    if(idbrand !=null)
    {
        location.href="?menu=" + menu + "&brand=" + idbrand +"&page=" + page + "&size=" + size;
        return;
    }
    location.href="?menu=" + menu + "&page=" + page + "&size=" + size;
}